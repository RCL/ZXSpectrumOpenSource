...
.fix_bit_pos:
            ...
            ; end with CF=0 (important for unpack routine)
    endm
...
.copy_chunk:
    ...
    DecodeNumber       ; context_index is already 257-1 as needed by decode_number
...
.keep_offset:
    ...
    DecodeNumber           ; length = upkr_decode_length(257 + 64);
    push    de
    exx
...
.state_b15_zero:
  ...
  ; CF=1 (by stop bit)
    ...
    ; [RCL] this code mostly serves as a progress indicator, although it is also responsible for setting the border to black before the main part starts (just happens that the last values decompressed are 0)
    push af
    and #07
    out (#fe), a
    pop af
    ; [RCL] end of border flashing code
...
.bit_is_0:
    ...
    if 0 ; [RCL] disabling the non-Z80N version as this intro supports Next only.
    ld      d,0
...
0:
  ...
  ENDIF
    ...
    else ; [RCL] this is the Z80N-optimized version multiplication

    ld d, h ;  D = (upkr_state>>8)
    ld e, a ;  E = state_scale
    mul
    ex hl, de   ; HL = state_scale * (upkr_state >> 8)
    ld e, a
    ld b, 0
    ld d, b
    add hl, bc   ; HL = state_scale * (upkr_state >> 8) + (upkr_state & 255)

    endif ; [RCL] end of Z80N specifics
...
*/
    ...
    DISPLAY "UPKR_PROBS_ORIGIN =",/A,UPKR_PROBS_ORIGIN
    ;IFDEF upkr.decode_number     ; if specific address is defined by user, move probs array there
    if (1)
probs:      EQU ((UPKR_PROBS_ORIGIN) + 255) & -$100     ; probs array aligned to 256
        DISPLAY "UPKR_PROBS_ORIGIN =",/A,UPKR_PROBS_ORIGIN, " probs=", /A, probs
    ELSE
probs:      EQU ($ + 255) & -$100                       ; probs array aligned to 256
        DISPLAY "SHOULD NOT ENTER! UPKR_PROBS_ORIGIN =",/A,UPKR_PROBS_ORIGIN, " probs=", /A, probs
    ENDIF
